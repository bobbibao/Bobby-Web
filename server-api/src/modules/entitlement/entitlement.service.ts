...
import {
  ...
  PLAN_ORDER,
} from '../model-catalog/model-catalog.defaults';
...
export class EntitlementService {
  ...
  ensureResolutionAllowed(
    ...
    if (invalid.length > 0) {
      // LOCAL DEV: Instead of throwing, just log warning for fallback handling
      console.warn(
        `[FALLBACK] Resolution ${normalizedRes} not allowed for models: ${invalid.join(
          ', ',
        )}. Continuing with fallback.`,
      );
      // Don't throw - let the fallback model handle it
      return;
    }
  }
